...
const familyDetail = {
  ...
  sourceMaps: [{ id: 20, runId: 7, canonicalCatalogItemId: 1, priceDisposition: "KEEP", imageDisposition: "KEEP", canonicalUpdatedAt: new Date("2026-08-29T00:00:00.000Z") }],
  canonicalMaps: [],
};
...
test("save returns field-level Portuguese errors for invalid aggregate", async () => {
  ...
  const aggregate = JSON.stringify({
    ...
    family: familyDetail,
    options: [],
    variants: [{ id: 4, clientKey: "variant-4", valueClientKeys: ["value-3"], sku: null, active: true, nameOverride: null, descriptionLinesOverride: null, unitPriceCents: null, imageId: null }],
  });
  ...
  await expect(response.json()).resolves.toMatchObject({
    ...
    fields: { options: expect.stringContaining("referenced") },
  });
});
...
test("archive uses expectedUpdatedAt", async () => {
  ...
  const response = await action(
    actionArgs({ intent: "archive", expectedUpdatedAt: "2026-08-29T00:00:00.000Z" }),
  );
